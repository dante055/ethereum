...
contract Voting {
    ...
    enum Status{NOTOPEN, OPEN, FINISHED}
    ...
    function beginVoting(uint _ballotId) external ballotExist(_ballotId) onlyAdmin() {
    ...
    function showBallots() external view returns(BallotTemp[] memory){
        BallotTemp[] memory _ballots = new BallotTemp[](nextBallotId);

        for(uint i = 0; i < nextBallotId; i++) {
            require(ballots[i].ballotExist == true);
            _ballots[i].ballotId = ballots[i].ballotId;
            _ballots[i].name = ballots[i].name;
            if(ballots[i].hasVotingBegin && block.timestamp >= ballots[i].endTime) {
                _ballots[i].ballotStatus = Status.FINISHED;
            } else if(ballots[i].hasVotingBegin ) {
                _ballots[i].ballotStatus = Status.OPEN;
            } else {
                _ballots[i].ballotStatus = Status.NOTOPEN;
            }
            _ballots[i].endTime = ballots[i].endTime;
        }
        return _ballots;
    } 
